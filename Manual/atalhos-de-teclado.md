...
Todos podem ser trocados na aba **Atalhos** — escolha outra tecla e, se quiser, combine com Ctrl/Alt/Shift. Se escolher uma **letra** como atalho, é **obrigatório** usar pelo menos um modificador (Ctrl, Alt ou Shift), para não atrapalhar os controles normais do jogo.

***Os atalhos só funcionam quando a janela do jogo está em foco (ou seja, quando a janela de configuração do Ranmza GT não está em primeiro plano). Assim você pode digitar normalmente nos campos da configuração sem disparar comandos sem querer.***
